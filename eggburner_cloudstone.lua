...
-------------------------------------------------------------------------------
local base = "nc_concrete_cloudstone.png"
local boxy = "nc_concrete_pattern_boxy.png^[opacity:140"
local horzy = "nc_concrete_pattern_horzy.png^[opacity:140"
...
local function burner(id, light, tile)
	minetest.register_node(modname .. ":incense_cloudstone_" ..id, {
		description = "Cloudstone Egg Burner",
		tiles = tile,
		drawtype = "nodebox",
		paramtype = "light",
		node_box = {
...
minetest.register_abm({
		label = "ignite cloudstone eggburner",
		interval = 10,
		chance = 2,
		nodenames = {modname.. ":incense_cloudstone_unlit"},
		neighbors = {"group:igniter", "group:torch_lit", "group:candle_lit"},
--		action_delay = true,
		action = function(pos)
			if not nodecore.quenched(pos) then
			minetest.set_node(pos, {name = modname.. ":incense_cloudstone_lit"})
			end
		end
	})
----------------QUENCHING-----------------
nodecore.register_abm({
		label = "Incense Cloudstone Quenching",
		interval = 0.1,
		chance = 1,
		nodenames = {modname .. ":incense_cloudstone_lit"},
		action = function(pos)
			if nodecore.quenched(pos) then
				nodecore.sound_play("nc_fire_snuff", {gain = 1, pos = pos})
				return minetest.set_node(pos, {name = modname.. ":incense_cloudstone_ashy"})
			end
		end
	})
nodecore.register_aism({
				label = "Held Incense Cloudstone Quenching",
				interval = 0.1,
				chance = 1,
				itemnames = {modname .. ":incense_cloudstone_lit"},
				action = function(stack, data)
						...
						if ext and nodecore.quenched(pos, data.node and 1 or 0.3) then
							nodecore.sound_play("nc_fire_snuff", {gain = 1, pos = pos})
							stack:set_name(modname .. ":incense_cloudstone_ashy")
							return stack
						end
				end
})
...
nodecore.register_aism({
				label = "Held Incense Cloudstone Use",
				interval = 90,
				chance = 10,
				itemnames = {modname .. ":incense_cloudstone_lit"},
				action = function(stack, data)
...
nodecore.register_craft({
		label = "refill eggburner_cloudstone",
		action = "pummel",
		wield = {name = "nc_tree:eggcorn"},
		after = rfcall,
		nodes = {
				{match = modname .. ":incense_cloudstone_empty", replace = modname .. ":incense_cloudstone_unlit"}
			}
	})
------------ASH DUMPING------------
nodecore.register_craft({
		label = "empty incense_cloudstone",
		action = "pummel",
		duration = 1,
		nodes = {
			{
				match = modname.. ":incense_cloudstone_ashy",
				replace = modname.. ":incense_cloudstone_empty"
			}
		},
		items = {
...
------------EGG DUMPING------------
nodecore.register_craft({
		label = "remove incense_cloudstone",
		action = "pummel",
		nodes = {
			{
				match = modname.. ":incense_cloudstone_unlit",
				replace = modname.. ":incense_cloudstone_empty"
			}
		},
		items = {
...
------------Break Burner-----------
nodecore.register_craft({
		label = "break incense cloudstone apart",
		action = "pummel",
		duration = 2,
		toolgroups = {thumpy = 3, cracky = 3, choppy = 3},
		nodes = {
			{
				match = {groups = {incense_cloudstone = true}},
...
------------CRAFT INCENSE------------
nodecore.register_craft({
	label = "chisel eggburner cloudstone",
	action = "pummel",
	duration = 2,
	toolgroups = {thumpy = 3},
	normal = {y = 1},
	...
	nodes = {
		...
		{
			y = -1,
			match = "nc_concrete:concrete_cloudstone_boxy",
			replace = modname .. ":incense_cloudstone_empty"
		}
	}
})
...
nodecore.register_ambiance({
		label = "Incense Cloudstone Ambiance",
		nodenames = {modname.. ":incense_cloudstone_lit"},
		interval = 1,
		chance = 2,
