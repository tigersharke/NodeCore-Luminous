...
local function burner(id, light, tile)
	minetest.register_node(modname .. ":incense_coalstone_" ..id, {
		description = "Tarstone Egg Burner",
		tiles = tile,
		drawtype = "nodebox",
		paramtype = "light",
		node_box = {
...
minetest.register_abm({
		label = "ignite eggburner_coalstone",
		interval = 10,
		chance = 2,
		nodenames = {modname.. ":incense_coalstone_unlit"},
		neighbors = {"group:igniter", "group:torch_coalstone_lit", "group:candle_coalstone_lit"},
--		action_delay = true,
		action = function(pos)
			if not nodecore.quenched(pos) then
			minetest.set_node(pos, {name = modname.. ":incense_coalstone_lit"})
			end
		end
	})
----------------QUENCHING-----------------
nodecore.register_abm({
		label = "Incense_coalstone Quenching",
		interval = 0.1,
		chance = 1,
		nodenames = {modname .. ":incense_coalstone_lit"},
		action = function(pos)
			if nodecore.quenched(pos) then
				nodecore.sound_play("nc_fire_snuff", {gain = 1, pos = pos})
				return minetest.set_node(pos, {name = modname.. ":incense_coalstone_ashy"})
			end
		end
	})
nodecore.register_aism({
				label = "Held Incense_coalstone Quenching",
				interval = 0.1,
				chance = 1,
				itemnames = {modname .. ":incense_coalstone_lit"},
				action = function(stack, data)
						...
						if ext and nodecore.quenched(pos, data.node and 1 or 0.3) then
							nodecore.sound_play("nc_fire_snuff", {gain = 1, pos = pos})
							stack:set_name(modname .. ":incense_coalstone_ashy")
							return stack
						end
				end
})
...
nodecore.register_aism({
				label = "Held Incense_coalstone Use",
				interval = 90,
				chance = 10,
				itemnames = {modname .. ":incense_coalstone_lit"},
				action = function(stack, data)
...
nodecore.register_craft({
		label = "refill eggburner_coalstone",
		action = "pummel",
		wield = {name = "nc_tree:eggcorn"},
		after = rfcall,
		nodes = {
				{match = modname .. ":incense_coalstone_empty", replace = modname .. ":incense_coalstone_unlit"}
			}
	})
------------ASH DUMPING------------
nodecore.register_craft({
		label = "empty incense_coalstone",
		action = "pummel",
		duration = 1,
		nodes = {
			{
				match = modname.. ":incense_coalstone_ashy",
...
------------EGG DUMPING------------
nodecore.register_craft({
		label = "remove incense_coalstone",
		action = "pummel",
		nodes = {
			{
				match = modname.. ":incense_coalstone_unlit",
				replace = modname.. ":incense_coalstone_empty"
			}
		},
		items = {
...
------------Break Burner-----------
nodecore.register_craft({
		label = "break incense_coalstone apart",
		action = "pummel",
		duration = 2,
...
------------CRAFT INCENSE------------
nodecore.register_craft({
	label = "chisel eggburner_coalstone",
	action = "pummel",
	duration = 2,
	toolgroups = {thumpy = 3},
	normal = {y = 1},
...
nodecore.register_ambiance({
		label = "Incense_coalstone Ambiance",
		nodenames = {modname.. ":incense_coalstone_lit"},
		interval = 1,
		chance = 2,
